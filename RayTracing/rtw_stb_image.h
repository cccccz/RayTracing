...
public:
    ...
    rtw_image(const char* image_filename) {
        ...
        size_t len;
        // Use _dupenv_s to retrieve the environment variable safely
        errno_t err = _dupenv_s(&imagedir, &len, "RTW_IMAGES");

        if (!err && imagedir != nullptr) {
            // If we have a valid directory, attempt to load the image
            if (load(std::string(imagedir) + "/" + image_filename)) {
                free(imagedir); // Free the memory allocated by _dupenv_s
                return;
            }
            free(imagedir); // Free the memory allocated by _dupenv_s
        }
